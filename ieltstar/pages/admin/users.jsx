...
export default function AdminUsers() {
  ...
  const openEdit = (user) => {
    ...
    setEditRoles(user.roles?.map((r) => r._id) || []);
    setEditOpen(true);
  };
  ...
  return (
    <>
      <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
        ...
        <Button startIcon={<Refresh />} variant="outlined" onClick={fetchUsers}>Refresh</Button>
      </Box>
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ bgcolor: "#0d1b3e" }}>
              {["Name", "Email", "Verified", "Roles", "Status", "Actions"].map((h) => (
          ...
          <TableBody>
            {users.map((u) => (
              <TableRow key={u._id} hover>
                ...
                <TableCell>
                  {u.roles?.map((r) => (
                    <Chip key={r._id} label={r.name} size="small" icon={<Shield />}
                      sx={{ mr: 0.5, bgcolor: r.name === "admin" ? "#e3f2fd" : "#f5f5f5" }} />
                  ))}
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  ...
                  <Button size="small" color="error" startIcon={<Delete />} onClick={() => deleteUser(u._id)}
                    disabled={u.email === "admin@gmail.com"}>Delete</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Edit Roles — {selectedUser?.fullName}</DialogTitle>
        <DialogContent>
          <FormControl fullWidth sx={{ mt: 1 }}>
            <InputLabel>Roles</InputLabel>
            <Select multiple value={editRoles} onChange={(e) => setEditRoles(e.target.value)}
              renderValue={(v) => v.map((id) => roles.find((r) => r._id === id)?.name).join(", ")}>
              {roles.map((r) => (
